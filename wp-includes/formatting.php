 ...
 * 将字符串清理为可用于 URL 或 HTML 属性的 slug。
 *
 * By default, converts accent characters to ASCII characters and further
 * limits the output to alphanumeric characters, underscore (_) and dash (-)
 * through the {@see 'sanitize_title'} filter.
 * 默认情况下，将重音字符转换为 ASCII 字符，并通过“sanitize_title”过滤器进一步将输出限制为字母数字字符、下划线 (_) 和破折号 (-)。
 *
 * If `$title` is empty and `$fallback_title` is set, the latter will be used.
 * 如果 $title 为空并且设置了 $fallback_title，则将使用后者。
 *
 * @since 1.0.0
 *
...
function esc_attr( $text ) {
     ...
     * 过滤已清理和转义的字符串以在 HTML 属性中输出。
	 *
	 * Text passed to esc_attr() is stripped of invalid or special characters
	 * before output.
     * 传递给 esc_attr() 的文本在输出之前被去除无效或特殊字符。
	 *
	 * @since 2.0.6
	 *
	 * @param string $safe_text The text after it has been escaped.
