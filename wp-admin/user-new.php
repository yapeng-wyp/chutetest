	...
	if ( is_multisite() ) {
		...
		switch ( $_GET['update'] ) {
			case 'newuserconfirmation':
				$messages[] = __( 'Invitation email sent to new user. A confirmation link must be clicked before their account is created.' );// 发送给新用户的邀请电子邮件。 在创建他们的帐户之前必须单击确认链接
				break;
			case 'add':
				$messages[] = __( 'Invitation email sent to user. A confirmation link must be clicked for them to be added to your site.' );// 发送给用户的邀请电子邮件。 必须单击确认链接才能将它们添加到您的站点。
				break;
			case 'addnoconfirmation':
				$message = __( 'User has been added to your site.' ); // 用户已经添加多你的站点
				...
				break;
			case 'addexisting':
				$messages[] = __( 'That user is already a member of this site.' ); // 该用户已经是该站点的成员
				break;
			case 'could_not_add':
				$add_user_errors = new WP_Error( 'could_not_add', __( 'That user could not be added to this site.' ) ); // 该用户不能添加此站点
				break;
			case 'created_could_not_add':
				$add_user_errors = new WP_Error( 'created_could_not_add', __( 'User has been created, but could not be added to this site.' ) ); // 用户已创建，但无法添加到此站点。
				break;
			case 'does_not_exist':
				$add_user_errors = new WP_Error( 'does_not_exist', __( 'The requested user does not exist.' ) );  // 请求的用户不存在
				break;
			case 'enter_email':
				$add_user_errors = new WP_Error( 'enter_email', __( 'Please enter a valid email address.' ) ); // 请输入有效的电子邮件地址
				break;
		}
	} else {
		if ( 'add' === $_GET['update'] ) {
...
<table class="form-table" role="presentation">
     ...
     * 评估上下文字符串以生成两种类型的新用户表单
     * 唯一可定位。 上下文是“添加现有用户”（多站点），
     * 和“添加新用户”（单个站点和网络管理员）。
	 *
	 * @since 3.7.0
	 *
...
<input name="action" type="hidden" value="createuser" />
    ...
    $new_user_countrycity       = $creating && isset( $_POST['country_city'] ) ? wp_unslash( $_POST['country_city'] ) : '';
...
<table class="form-table" role="presentation">
    ...
    <tr class="form-field">
        <th scope="row"><label for="country_city"><?php _e( 'Country City' ); ?> </label></th>
        <td><input name="country_city" type="text" id="country_city" value="<?php echo esc_attr( $new_user_countrycity ); ?>" /></td>
    </tr>
    <tr class="form-field">
