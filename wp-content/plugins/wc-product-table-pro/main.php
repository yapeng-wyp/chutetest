<?php
/*
 * Plugin Name: WooCommerce Product Table PRO
 * Plugin URI: https://wcproducttable.com/
 * Description: Display your WooCommerce products in beautiful, mobile responsive, fully customizable tables.
...
function wcpt_enqueue_scripts (){
  ...
  // multirange
  wp_enqueue_script( 'multirange',  plugin_dir_url( __FILE__ ) . 'assets/multirange/js.js', 'jquery', WCPT_VERSION, true );
  wp_enqueue_style( 'multirange',  plugin_dir_url( __FILE__ ) . 'assets/multirange/css.css', false, WCPT_VERSION );
  ...
  wp_localize_script( 'wcpt', 'wcpt_params', array(
    'ajax_url' => admin_url( 'admin-ajax.php' ),
  ...
  switch ( $theme_slug ) {
    ...
    case 'bavarian':
      ...
      ?>
      jQuery(function($){
        $('body').on('click', 'a.wcpt-title, .wcpt-button-product_link, a.wcpt-product-image-wrapper, .wcpt-product-link', function(e){
...
add_action('wp_loaded', 'wcpt_cart_works', 15);
function wcpt_cart_works(){
  if(
    empty( $_REQUEST['wcpt_payload'] ) ||
    empty( $_REQUEST['wcpt_payload']['products'] )
  ){
    return;
  }
        ...
        if( $item['product_id'] == $product_id ){
          ...
          if( $item['variation_id'] ){
            if( 
              ...
              $qty = $_REQUEST['wcpt_payload']['variations'][$product_id][ $item['variation_id'] ];
...
function wcpt_shortcode_product_table($atts= array()){
  ...
  if( $error_message = wcpt_sc_error_checks( $GLOBALS['wcpt_table_data'], $atts ) ){
    ...
    if( ! empty( $GLOBALS['wcpt_table_instance'] ) ){
      $prev_global__wcpt_table_instance = $GLOBALS['wcpt_table_instance'];
    }
    if( ! empty( $GLOBALS['wcpt_table_data'] ) ){
      $prev_global__wcpt_table_data = $GLOBALS['wcpt_table_data'];
    }
    ...
    if( ! empty( $prev_global__wcpt_table_instance ) ){
      $GLOBALS['wcpt_table_instance'] = $prev_global__wcpt_table_instance;
    }
    if( ! empty( $prev_global__wcpt_table_data ) ){
      $GLOBALS['wcpt_table_data'] = $prev_global__wcpt_table_data;
    }
...
// wcpt price
function wcpt_price( $price, $trim_zeros= false, $safari_fix= true ){
  ...
  if( 
    ...
    if( $currency_symbol == '&#036;' ){
      ...
      $html_class = 'wcpt-safari-currency';      
    }
    $currency = '<span class="wcpt-currency '. $html_class .'">' . $currency_symbol . '</span>';    
  }else{
...
function wcpt_safari_dollar_fix( $elm_markup, $elm ){
  ...
  if( 
    ...
    wcpt_icon('dollar-sign');
    $currency_symbol = ob_get_clean();
    $currency = '<span class="wcpt-currency wcpt-safari-currency">' . $currency_symbol . '</span>';

    return str_replace( '<span class="woocommerce-Price-currencySymbol">&#36;</span>', $currency, $elm_markup );
  }
  return $elm_markup;
}
...
function wcpt_get_price_to_display( $product= null ){
  ...
  if( apply_filters( 'wcpt_product_is_on_sale', $product->is_on_sale(), $product ) ){
    $price = apply_filters('wcpt_product_get_sale_price', $product->get_sale_price(), $product);
  }else{
    $price = apply_filters('wcpt_product_get_regular_price', $product->get_regular_price(), $product);
  }
...
function wcpt_parse_navigation ( $data= false ){
  ...
  ?>
  <div 
    class="<?php echo apply_filters('wcpt_nav_header_class', 'wcpt-navigation wcpt-header {{maybe-always}}'); ?>"
      ...
      function start_el( &$output, $category, $depth = 0, $args = array(), $id = 0 ){
        ...
        if( $this->args['redirect'] ){
          ...
          >
            ...
            >
              <a 
                href="<?php echo strtok( get_term_link( $category->term_taxonomy_id ), '?' ) . wcpt_get_archive_query_string( 'category', $category->term_taxonomy_id ); ?>" 
...
function wcpt_general_placeholders__print_placeholders ($destination= false){
  ...
  <small style="cursor: default;">
    <strong>Available placeholders:</strong> <?php wcpt_pro_badge(); ?><br>    
...
function wcpt_cart(){
  // 'wcpt_cart_works' adds items to cart beforehand at wp_loaded
