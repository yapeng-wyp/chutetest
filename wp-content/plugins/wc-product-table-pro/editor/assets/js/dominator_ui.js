  ...
  dominator_ui = {
    ...
    init: function($elm, data){
      ...
      if( $elm.attr('wcpt-initial-data') ){
        ...
        var initial_data = $.extend( {}, _.initial_data[ $elm.attr('wcpt-initial-data') ] );
      ...
      $panel_conditions.each(function(){
        var $this         = $(this),
            ancestor_skip = $this.attr('wcpt-panel-ancestor-skip'),
            $data_node    = $this.parent().closest('[wcpt-model-key]');
        ...
        while( ancestor_skip > 0 ){
          $data_node  = $data_node.parent().closest('[wcpt-model-key]');
      ...
      $controllers.add($data_nodes).each(function(){
        ...
        $children.filter('[type="radio"]').each(function(){
          ...
          var name = 'wcpt-' + model_key + '-' + Math.random(0, 1.0e+9);
      ...
      $duplicate_row.off('click').on('click', function(){
        ...
        if( $row.hasClass('wcpt-toggle-column-expand') ){
          $template.addClass('wcpt-toggle-column-expand');
        }else{
          $template.removeClass('wcpt-toggle-column-expand');
        }
