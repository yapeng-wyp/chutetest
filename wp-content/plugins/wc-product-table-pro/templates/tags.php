    ...
    foreach( $relabels as $rule ){
      if( 
        ...
        $match = true;
				...
				// wrap in a / div tag
				if( $is_link ){
					$output .= '<a class="wcpt-tag wcpt-taxonomy-term ' . $term_html_class . '" '. $common_data_attrs .'>' . $label . '</a>';
				}else{
					$output .= '<div class="wcpt-tag wcpt-taxonomy-term ' . $term_html_class . '" '. $common_data_attrs .'>' . $label . '</div>';
				}
		...
		if( ! $match ){
			if( $is_link ){
				$output .= '<a class="wcpt-tag wcpt-taxonomy-term" '. $common_data_attrs .'>' . $term->name . '</a>';
			}else{
				$output .= '<div class="wcpt-tag wcpt-taxonomy-term" '. $common_data_attrs .'>' . $term->name . '</div>';
			}
		}
...
if( 
	...
	! wcpt_check_if_nav_has_filter( null, 'tags_filter', false ) 
){
	$click_action = false;
}
...
if( ! empty( $output ) ){
	?>
		<div
			class="wcpt-tags <?php echo $html_class; ?>"
